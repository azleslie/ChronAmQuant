...
The first step is to download a newspaper batch [from Chronicling America](https://chroniclingamerica.loc.gov/ocr/). The batch used here is called `allspice`; it consists of the *Perth Amboy Evening News* from March 1903 to March 1907. Once downloaded, unzip the tarball; this will take a few more minutes. The result will show up as a single directory with the name "sn85035720": this is Chronicling America's ID for the *Perth Amboy Evening News*, so we'll leave it that way. Any other single-paper Chronicling America batch will do just as well if you'd prefer to work with data from a different newspaper in a different time or place. Just make sure that [1] the batch includes enough page data to produce a good number of search results and [2] you change all instances of "sn85035720" to the ID of whatever other paper you choose.
...
Now let's turn to the *Perth Amboy Evening News*. Chronicling America datasets come highly structured; open up your downloads folder and familiarize yourself with it for a bit. You'll notice the file directory is structured something like this: "/Users/elf/Downloads/[paper ID]/[year]/[month]/[day]/[edition]/[page]". Once you get all the way to the bottom of things, you'll see two files: a plain old .txt file and an .xml file that indicates the position of the text on the page. 

The .txt file is the only one we want - and in fact, we *do not* want the .xml files, which take up tons of space. This next line of code uses wildcards to remove all files with the .xml extension within the exact number of directories specified by asterisk. Change the names or number of the directories prior to "sn85035720" to match the file path on your computer, but **do not** remove "sn85035720" (unless replacing it with the ID for a different newspaper you downloaded instead) or change anything after it: we don't want to go accidentally removing other things from your computer.

```{r}
unlink(Sys.glob(file.path("/Users/elf/Downloads/sn85035720/*/*/*/*/*", "*.xml")))
```
...
for (k in seq_along(years)) {
  ...
  for (j in seq_along(months)) {
    days <- list.files(paste("/Users/elf/Downloads/sn85035720/", 
...
for (k in seq_along(years)) {
  ...
  for (j in seq_along(months)) {
    ...
    for (i in seq_along(days)) {
      pages <- list.files(Sys.glob(paste("/Users/elf/Downloads/sn85035720/", 
...
Great: now we have a matrix of all the metadata we want to retain from the Chronicling America file structure. 

(If we were working with multiple newspapers, we would start by using `list.files` to find all the directories in our downloads that start with "sn." Then we would wrap an additional `for` loop around this one.)
...
searchfun <- function (year, month, day, page) { 
  ...
  file_path <- paste("/Users/elf/Downloads/sn85035720/", 
  ...
  tot_pg <- length(list.files(Sys.glob(paste("/Users/elf/Downloads/sn85035720/", 
                     year, "/", month, "/", day, "/*/", sep=""))))
