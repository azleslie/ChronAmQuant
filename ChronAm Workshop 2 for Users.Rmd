...
There is a substantial difference in the number of pages the *Perth Amboy Evening News* printed per issue even between 1903 and 1907, the first and last year of our data - yet another reason why the raw number of references is limited. This means that while we have the number of pages in each issue for which there was a search hit, we'll need to go back to our filepaths in order to pick up the exact number of pages printed per month. You'll recognize the code we would use to do this as a slight adaptation of what we used in the previous workshop to generate an input matrix.

```{r, eval=FALSE}
years <- list.files("/Users/elf/Downloads/sn85035720")
for (k in seq_along(years)) {
  months <- list.files(paste("/Users/elf/Downloads/sn85035720/", years[k], sep=""))
  for (j in seq_along(months)) {
    ...
    pages <- length(list.files(Sys.glob(paste("/Users/elf/Downloads/sn85035720/", 
...
If you no longer have this data on your computer for whatever reason (e.g., the shared lab computers for our workshop re-image every night), don't sweat: I've saved the results as a .csv file, which we can read in instead.

```{r}
by_month <- read.csv("sn85035720_date+pages.csv") %>% select(-X)
...
We can generate a visualization to graph what percentage of references to the search phrase appears on each page to further explore page distribution. (Don't forget `ggtitle`.)
...
With a bit of extra fiddling, we can generate a visualization to compare the distribution of references by page for multiple search phrases. If you downloaded all these files you'll be able to run this code yourself; if not, you can see the results in the .pdf file for this workshop.

```{r, echo=FALSE, warning=FALSE}
hits1 <- read.csv("andrew_carnegie_sn85035720.csv") %>% mutate(Content = "Andrew Carnegie")
hits1 <- hits1 %>% group_by(Content, Page) %>% summarize(Total=n()) %>% mutate(Page_Percents = Total/sum(Total)*100) 
hits2 <- read.csv("theodore_roosevelt_sn85035720.csv") %>% mutate(Content = "Theodore Roosevelt")
hits2 <- hits2 %>% group_by(Content, Page) %>% summarize(Total=n()) %>% mutate(Page_Percents = Total/sum(Total)*100)
hits3 <- read.csv("russo_japanese_war_sn85035720.csv") %>% mutate(Content = "Russo-Japanese War") 
hits3 <- hits3 %>% group_by(Content, Page) %>% summarize(Total=n()) %>% mutate(Page_Percents = Total/sum(Total)*100)
hits4 <- read.csv("spanish_american_war_sn85035720.csv") %>% mutate(Content = "Spanish-American War")
hits4 <- hits4 %>% group_by(Content, Page) %>% summarize(Total=n()) %>% mutate(Page_Percents = Total/sum(Total)*100)
hits5 <- read.csv("booker_t_washington_sn85035720.csv") %>% mutate(Content = "Booker T. Washington")
...
But reprints aren't semantically meaningless!^[See Ryan Cordell and David A. Smith's [Viral Texts Project](http://viraltexts.org/) and M. H. Beals' [Scissors and Paste Database](http://scissorsandpaste.net/) for two important, ongoing efforts to understand the significance of reprinting in nineteenth-century newspapers. See also McGill, Meredith. *American Literature and the Culture of Reprinting, 1834-1853*. Philadelphia: University of Pennsylvania Press, 2003.] They should never be excluded from data; rather, we should try to ascertain what kind of content and conditions cause an article referencing our name or phrase to be reprinted.

To begin to do so, we can graph temporal distribution again with the addition of designating uniqueness as a categorical variable. When working with enough newspapers, this would illustrate the apprxoimate proportion of original references. (Don't forget `ggtitle`.)
